...
SELECT REVERSE(first_name), REVERSE(last_name), birthdate - 2 , height + 10, country_id, (SELECT id FROM countries WHERE name='Armenia') as country_id
FROM actors
WHERE id <= 10;
...
#6
SELECT mai.id, m.title, runtime, budget, release_date 
FROM movies_additional_info AS mai
LEFT JOIN movies AS m ON m.movie_info_id = mai.id
...
(2022 - (EXTRACT(YEAR FROM birthdate))) AS age, height
FROM actors
LEFT JOIN movies_actors AS ma ON id = ma.actor_id
